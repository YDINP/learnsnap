  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-2 mb-4">
        <Link href={`/category/${card.category}`} className="text-sm text-gray-600 hover:text-gray-400">
          ← {cat?.label}
        </Link>
      </div>
        ...
        <div className="p-6 border-b border-gray-800">
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="flex gap-2 text-xs">
              <span className="bg-gray-800 text-gray-400 px-2 py-1 rounded-full">{difficultyLabel}</span>
              <span className="bg-indigo-950 text-indigo-400 px-2 py-1 rounded-full">{card.estimatedMinutes ?? 3}분</span>
            </div>
          </div>
          <h1 className="text-2xl font-bold text-white mb-1">{card.title}</h1>
          {card.learningGoal && (
            <p className="text-sm text-indigo-400 font-medium">🎯 {card.learningGoal}</p>
          )}
          {card.tags.length > 0 && (
      ...
      <div className="text-center mt-6">
        <Link
          href="/feed"
          className="inline-block bg-indigo-600 text-white px-6 py-3 rounded-2xl text-sm font-medium hover:bg-indigo-500 transition-colors"
        >
          스와이프 피드로 더 보기 ↕
        </Link>
      </div>
