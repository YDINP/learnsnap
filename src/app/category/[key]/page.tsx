...
export default async function CategoryPage({ params }: Props) {
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8">
      <Link href="/" className="text-sm text-gray-400 hover:text-gray-600 mb-4 inline-block">← 홈</Link>
        ...
        <div className="space-y-3">
          {cards.map(card => (
            ...
            >
              ...
              <div className="text-xs text-gray-400 shrink-0">{card.estimatedMinutes ?? 3}분</div>
            </Link>
          ))}
        </div>
      )}
